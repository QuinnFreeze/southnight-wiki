...
class AuthController extends Controller
{
    ...
    public function register(Request $request) { $data=$request->validate(['username'=>'required|string|min:3|max:24|unique:users,username','email'=>'required|email|max:255|unique:users,email','password'=>'required|string|min:10|confirmed']); $data['name']=$data['username']; $data['password']=password_hash($data['password'], PASSWORD_DEFAULT); unset($data['password_confirmation']); $data['role']=User::count()===0?'admin':'user'; $data['status']='active'; $user=User::createWithUniqueUid($data); Audit::record('user.register','user',(string)$user->id); auth()->login($user); $request->session()->regenerate(); return redirect()->route('account')->with('status','注册成功。'); }
